 ...
 */
function transformRenderBlock(node) {
    // Defensive: validate input
    if (!node || typeof node !== 'object') {
    ...
    const args = node.args && Array.isArray(node.args)
        ? node.args.map((arg) => extractExpression(arg))
        : undefined;
    return {
        type: 'render',
    ...
    function extract(n, depth = 0) {
        ...
        if (n.type === 'ArrayExpression') {
            const elements = Array.isArray(n.elements)
                ? n.elements.map((el) => el ? extract(el, depth + 1) : '').join(', ')
                : '';
            return `[${elements}]`;
        }
 ...
 */
function prefixExpression(expr) {
    // Defensive: validate input
 ...
 * Transform {@const} tag
 */
function transformConstTag(node) {
    return {
        type: 'const',
        name: node.name,
 ...
 * Transform {@html} tag
 */
function transformHtmlTag(node) {
    return {
        type: 'html',
        expression: extractExpression(node.expression)
    };
 ...
 * Transform {@debug} tag
 */
function transformDebugTag(node) {
    return {
        type: 'debug',

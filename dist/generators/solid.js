...
function generateSolid(ir) {
    ...
    // Combine all parts
    const code = (0, code_gen_1.joinStatements)(solidImports, externalImports, types, propsInterface, component);
    return {
        code
    };
}
 ...
 */
function generateSolidImports(ctx) {
 ...
 */
function generatePropsInterface(ir) {
 ...
 */
function generateComponent(ir, ctx) {
 ...
 */
function generatePropsDestructure(ir) {
 ...
 */
function generateRefDeclarations(ir, ctx) {
    if (!ir.refs || ir.refs.length === 0)
        return '';
    ...
    const declarations = ir.functions.map((func) => {
        ...
        for (const state of ir.state) {
            ...
            if (setter) {
                ...
                body = body.replace(new RegExp(`\\b${state.name}\\+\\+`, 'g'), `${setter}(${getter}() + 1)`);
                body = body.replace(new RegExp(`\\b${state.name}--`, 'g'), `${setter}(${getter}() - 1)`);
                // Replace count = value with setCount(value)
                // Need to be careful to handle expressions that might contain state values
                body = body.replace(new RegExp(`\\b${state.name}\\s*=\\s*([^=].+?);`, 'g'), (match, value) => {
                    // Transform state references in the value to use getters
                    let transformedValue = value;
                    for (const s of ir.state) {
                        transformedValue = transformedValue.replace(new RegExp(`\\b${s.name}\\b`, 'g'), `${s.name}()`);
                    }
        ...
        for (const state of ir.state) {
            ...
            body = body.replace(new RegExp(`\\b${state.name}(?!\\()\\b`, 'g'), `${state.name}()`);
        }
 ...
 */
function generateJSX(node, ctx, depth = 0) {
    switch (node.type) {
        case 'element':
...
function generateElementJSX(node, ctx, depth) {
    ...
    for (const [key, value] of Object.entries(bindings)) {
        ...
        if (key === 'class') {
            props.push(`className={${transformExpression(valueStr, {}, ctx)}}`);
        }
        else {
            props.push(`${key}={${transformExpression(valueStr, {}, ctx)}}`);
        }
    }
    // Handle attributes
...
function generateSlotJSX(node, ctx, depth) {
    ...
    const meaningful = fallback.filter((child) => !isEmptyFallbackChild(child));
...
function isEmptyFallbackChild(node) {
 ...
 */
function generateSlotFallbackExpression(node, ctx, depth) {
    if (node.type === 'text') {
 ...
 */
function generateEachJSX(node, ctx, depth) {
 ...
 */
function transformExpressionInJSX(expr, ctx) {
    ...
    if (ctx.stateGetters) {
        for (const [stateName] of ctx.stateGetters) {
            ...
            transformed = transformed.replace(new RegExp(`\\b${stateName}(?!\\()\\b`, 'g'), `${stateName}()`);
        }
    }
    // Add accessors for derived values (memos also need to be called)
    if (ctx.derivedNames) {
        for (const derivedName of ctx.derivedNames) {
            transformed = transformed.replace(new RegExp(`\\b${derivedName}(?!\\()\\b`, 'g'), `${derivedName}()`);
        }
    }
    return transformed;
}
 ...
 */
function transformExpression(expr, ir, ctx) {
    // Remove IR prefixes (state., props., derived., functions.)
    let transformed = expr;
    // First remove the prefixes
    transformed = transformed.replace(/\bstate\./g, '');
    ...
    if (ir.derived) {
        for (const derived of ir.derived) {
            transformed = transformed.replace(new RegExp(`\\b${derived.name}(?!\\()\\b`, 'g'), `${derived.name}()`);
        }
    }
    return transformed;
}
...
function generateDceElementJSX(node, ctx, depth) {
    ...
    const component = transformExpression(tagExpression, {}, ctx);
    // Collect all props
    const props = [];

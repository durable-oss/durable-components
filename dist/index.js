...
function compile(source, options) {
    ...
    try {
        ...
        let jsCode = String(file);
        if (options.browserSafe) {
            if (options.target === 'vue') {
                // Re-generate using browser-compatible options API format
                jsCode = (0, vue_1.generateVue)(tree, { browserSafe: true }).code;
            }
            else {
                jsCode = jsCode
                    .split('\n')
                    ...
                    .join('\n')
                    .replace(/^export (function |class |default |)/gm, '$1');
            }
        }
        const js = { code: jsCode };
